---
description: Retoma um projeto que já usa o Boare Protocol Dev
---

Leia docs/CONTINUAR.md e siga a próxima ação recomendada. Compare com os artefatos reais em docs/. Se esse arquivo não existir, leia o primeiro link que conseguir acessar, descubra o passo atual pelo que existe em docs/ e crie docs/CONTINUAR.md antes de avançar:
1. https://raw.githubusercontent.com/pauloboare/BoareProtocolDev/v1/CONDUZIR.md
2. https://cdn.jsdelivr.net/gh/pauloboare/BoareProtocolDev@v1/CONDUZIR.md
3. https://github.com/pauloboare/BoareProtocolDev/blob/v1/CONDUZIR.md
